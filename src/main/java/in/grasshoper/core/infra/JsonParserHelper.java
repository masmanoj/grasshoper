...
public class JsonParserHelper {
    ...
    public static LocalDate convertFromDateFormat(final String dateAsString, final String parameterName, final String dateFormat) {
        ...
        if (StringUtils.isNotBlank(dateAsString)) {
            try {
                eventLocalDate = DateTimeFormat.forPattern(dateFormat).parseLocalDate(dateAsString.toLowerCase());
            } catch (final IllegalArgumentException e) {
                final List<ApiParameterError> dataValidationErrors = new ArrayList<ApiParameterError>();
                final ApiParameterError error = ApiParameterError.parameterError("validation.msg.invalid.date.format", "The parameter "
                        + parameterName + " is invalid based on the dateFormat: '" + dateFormat + "' provided:", parameterName,
